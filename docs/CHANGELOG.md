...
- **ODEC-025: Discovery Mode B — DESIGN-seeded Capabilities**
  - Discovery agent now supports two modes: A (standalone) and B (DESIGN-seeded)
  - Mode B activated when `manifest.yaml` exists in inputs directory
  - Loads pre-resolved capabilities from DESIGN pipeline as seed
  - Merges seed capabilities with prompt-discovered capabilities (seed has precedence)
  - Extracts service_name from manifest.context_id in Mode B
  - Applies tech_defaults from manifest as variant_selections
  - New detection_reason: "design-seeded" for manifest capabilities
  - Output includes `discovery_mode` field for traceability
  - Orchestrator updated to detect manifest and extract service name
  - Backward compatible: Mode A behavior unchanged when no manifest present
