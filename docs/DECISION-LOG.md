...
- [ODEC-016](#odec-016) - Module Dependency Metadata
- [ODEC-024](#odec-024) - Deterministic Template Skip via Config Flags
- [ODEC-025](#odec-025) - Discovery Mode B — DESIGN-seeded Capabilities
- [ODEC-019](#odec-019) - Static Template Lint (approved, deferred)
...
### ODEC-019: Static Template Lint

**Status:** ✅ Approved, implementation deferred. See full entry below (Pending Decisions section).
...
## ODEC-016: Module Dependency Metadata {#odec-016}

**Fecha:** 2026-02-06  
**Estado:** ✅ Implementado

**Contexto:**  
Los módulos no declaraban formalmente sus dependencias inter-módulo. Esto impedía que el Plan Agent agrupara módulos en subphases automáticamente cuando una fase tiene más de 4 módulos. Además, sin `requires` explícito, el orden de generación dependía de convenciones implícitas.

**Problema:**
- Sin metadata de dependencias, el Plan Agent no puede optimizar subphase grouping
- Al añadir nuevas features (cache, etc.) se puede superar el threshold de 4 módulos por fase
- No hay validación de que un módulo se genera después de sus dependencias

**Decisión:** Añadir sección `dependencies` al frontmatter de cada MODULE.md:
...
dependencies:
  ...
  layer: <string>     # Architectural layer (informational)
```

**Campos:**
- `requires`: Hard constraint — Plan Agent must schedule required modules in earlier subphases
- `co_locate`: Soft hint — Plan Agent groups these together when possible (import coherence)
- `incompatible`: Hard constraint — Discovery Agent rejects invalid combinations
- `layer`: Informational metadata, not consumed by pipeline

**Implementación:**
- 10/10 MODULE.md actualizados con sección `dependencies`
- AUTHORING MODULE.md actualizado con documentación completa
- AUTHORING README.md index actualizado

**Archivos modificados:**
- `modules/mod-code-*/MODULE.md` (10 archivos)
- `model/standards/authoring/MODULE.md`
- `model/standards/authoring/README.md`
...
### ODEC-019: Static Template Lint (approved, deferred)
...
**Scope:** Authoring tooling — runs when architects create/modify modules, not during generation pipeline. Lives in `runtime/validators/authoring/validate-templates.sh`. To be documented in AUTHORING TEMPLATE.md as mandatory pre-commit step.

**Niveles de implementación:**
- **Nivel 1 (priority):** Static lint — cross-reference imports vs outputs across modules (0 tokens, catches 80% of bugs)
- **Nivel 2 (future):** Cross-module contracts via `contracts.yaml` per module
- **Nivel 3 (future):** Dry-run compilation with dummy variable substitution

**Status:** Approved. Implementation deferred — the Compilation Gate (ODEC-023) provides runtime safety net. Template lint adds value primarily when adding new features to KB.
...
## ODEC-024: Deterministic Template Skip via Config Flags {#odec-024}

**Fecha:** 2026-02-06  
**Estado:** ✅ Implementado

**Contexto:**  
DEC-035 introduced the Config Flags Pub/Sub pattern where capabilities publish flags (e.g., `hateoas: true`) and modules subscribe and react. However, the "skip template" reaction was not deterministic — it relied on a comment in the template header that the LLM had to interpret:
...
This worked in practice because the PoC always activates HATEOAS, but if someone generates without HATEOAS, there's no guarantee the LLM would correctly include/exclude the template.

**Problema:**
- Template skip based on config flags was probabilistic (LLM interpretation)
- Variant filtering (`// Variant:` header) was already deterministic in run-codegen.sh
- Inconsistency: one mechanism deterministic, the other probabilistic

**Decisión:** Extend run-codegen.sh to read `subscribes_to_flags` from MODULE.md and apply `skip_when` filtering deterministically.

**Diseño:**

```
1. Before template loop: read MODULE.md for each module in subphase
2. Parse subscribes_to_flags → extract (flag, template, skip_when)
3. Cross-reference with config_flags from generation-context.json
4. Build skip_templates set: {(module_id, template_rel_path)}
5. During template loop: check skip set before including template
```

**KB Changes (MODULE.md format):**
  ...
  - flag: hateoas
    ...
    affects:
      - template: templates/application/dto/Response.java.tpl
        skip_when: true       # Deterministic: skip when flag is true
    behavior: |               # Human-readable documentation (not parsed)
      When hateoas=true: skip. When hateoas=false: generate.
```

**Orchestration Changes (run-codegen.sh):**
- Reads MODULE.md YAML frontmatter for `subscribes_to_flags`
- Parses `skip_when` field for each affected template
- Applies deterministic filtering before template inclusion
- Reports skipped templates in console output

**Principios:**
- Stack-agnostic: any module can declare skip_when for any flag
- Model-native: skip_when is declared in MODULE.md (KB), executed in pipeline (Orchestration)
- Backward-compatible: modules without subscribes_to_flags are unaffected
- Two mechanisms coexist: `// Variant:` for intra-module variants, `skip_when` for cross-module flag reactions

**Archivos modificados:**
- `scripts/run-codegen.sh` — Added skip_when filtering logic
- KB: `modules/mod-code-015/MODULE.md` — Updated subscribes_to_flags format with skip_when
- KB: `model/standards/authoring/MODULE.md` — Documented skip_when mechanism

---

## ODEC-025: Discovery Mode B — DESIGN-seeded Capabilities {#odec-025}

**Fecha:** 2026-02-20
**Estado:** ✅ Implementado

**Contexto:**
The DESIGN pipeline (phases 0→4) produces a manifest.yaml per bounded context with pre-resolved capabilities. CODE pipeline needs to accept these as seed and merge with prompt-discovered capabilities. See KB DEC-085.

**Problema:**
- Discovery agent only operated in "Mode A" (prompt-only)
- No mechanism to receive pre-resolved capabilities from DESIGN bridge
- DESIGN resolves capabilities (what), CODE resolves modules (how) — responsibility split needed

**Decisión:** Dual-mode discovery with merge-with-precedence.

**Mode A (Standalone):** No changes. Discovery from prompt only. Backward compatible.

**Mode B (DESIGN-seeded):** Activated when `manifest.yaml` exists in inputs_dir:
1. Load seed capabilities from manifest (pre-validated by DESIGN)
2. Extract service_name from `manifest.context_id`
3. Extract tech_defaults for variant_selections
4. Run normal R1-R9 discovery on prompt for additional capabilities
5. Merge: seed + discovered, no duplicates (seed has precedence)
6. Resolve modules from final capability set (always CODE responsibility)
7. Mark seed capabilities with `detection_reason: "design-seeded"`
8. Output includes `discovery_mode: "A"|"B"` for traceability

**Key principle:** DESIGN resolves **capabilities**, CODE resolves **modules**. The manifest contains capability IDs but NOT module IDs.

**Archivos modificados:**
- `scripts/run-discovery.sh` — Mode detection, manifest injection, merge rules in system prompt
- `scripts/orchestrate.sh` — Service name extraction from manifest (priority over spec/prompt)
- `agents/discovery-agent.md` — Documented Mode A/B, merge algorithm

**Impacto en downstream agents:**
- Context Agent: No changes needed (receives discovery-result.json as before)
- Plan Agent: No changes needed (plans based on capabilities/modules from discovery)
- CodeGen Agent: No changes needed (generates based on plan)
- The discovery-result.json schema is backward compatible (new fields are additive)
