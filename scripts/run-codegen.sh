...
# DEC-035/ODEC-016: Build skip set from subscribes_to_flags in MODULE.md
# Reads each module's MODULE.md, parses subscribes_to_flags, and builds a set of
# template paths to skip based on config_flags values.
# This is stack-agnostic: any module can declare skip_when for any flag.
import yaml

skip_templates = {}  # key: (module_id, template_rel_path) → reason

for module in subphase['modules']:
    module_id = module['module_id']
    module_md_path = os.path.join(kb_dir, f"modules/{module_id}/MODULE.md")
    if not os.path.isfile(module_md_path):
        continue
    
    with open(module_md_path, 'r') as mf:
        module_content = mf.read()
    
    # Extract YAML frontmatter(s) - may have multiple --- blocks
    # Look for subscribes_to_flags in the content
    if 'subscribes_to_flags:' not in module_content:
        continue
    
    # Parse the subscribes_to_flags section using regex to extract structured data
    # Find the subscribes_to_flags block
    stf_match = re.search(
        r'subscribes_to_flags:\s*\n((?:[ \t]+.*\n)*)',
        module_content
    )
    if not stf_match:
        continue
    ...
    try:
        stf_yaml = yaml.safe_load('subscribes_to_flags:\n' + stf_match.group(1))
        subscriptions = stf_yaml.get('subscribes_to_flags', [])
    except:
        continue
    
    for sub in subscriptions:
        flag_name = sub.get('flag', '')
        flag_value = config_flags.get(flag_name)
        
        if flag_value is None:
            continue  # Flag not set, no action
        
        affects = sub.get('affects', [])
        for affect in affects:
            if isinstance(affect, dict):
                tpl_path = affect.get('template', '')
                skip_when = affect.get('skip_when')
                
                if skip_when is not None:
                    # Compare: skip_when can be bool or string
                    skip_when_val = str(skip_when).lower() in ('true', '1', 'yes')
                    flag_bool = str(flag_value).lower() in ('true', '1', 'yes')
                    
                    if skip_when_val == flag_bool:
                        # Normalize template path: remove leading "templates/" if present
                        norm_path = re.sub(r'^templates/', '', tpl_path)
                        skip_templates[(module_id, norm_path)] = f"flag {flag_name}={flag_value}, skip_when={skip_when}"

if skip_templates:
    print(f"  DEC-035 skip_when: {len(skip_templates)} template(s) will be skipped by config flags")
...
with open(prompt_file, 'a') as pf:
    ...
    for module in subphase['modules']:
        ...
        for tpl_file in tpl_files:
            ...
            # DEC-035: Check skip_when from subscribes_to_flags
            if (module_id, rel_path) in skip_templates:
                reason = skip_templates[(module_id, rel_path)]
                skipped_flags += 1
        ...
        if skipped_variants > 0:
            skip_parts.append(f"{skipped_variants} non-{http_client_variant} variants")
        if skipped_flags > 0:
            skip_parts.append(f"{skipped_flags} by config flags")
        
        if skip_parts:
            print(f"  {module_id}: {included_count} templates (skipped {', '.join(skip_parts)})")
        else:
