...
# ─────────────────────────────────────────────────────────────────────────────
# MODE DETECTION: Mode A (standalone) vs Mode B (DESIGN-seeded)
# ─────────────────────────────────────────────────────────────────────────────
MANIFEST_FILE="${INPUTS_DIR}/manifest.yaml"
if [ -f "${MANIFEST_FILE}" ]; then
    DISCOVERY_MODE="B"
    echo "Mode:    B (DESIGN-seeded — manifest.yaml detected)"
    # Extract service name and context from manifest
    MANIFEST_CONTEXT=$(python3 -c "import yaml; m=yaml.safe_load(open('${MANIFEST_FILE}')); print(m.get('context_id','unknown'))")
    MANIFEST_STACK=$(python3 -c "import yaml; m=yaml.safe_load(open('${MANIFEST_FILE}')); print(m.get('tech_stack','java-spring-boot-3'))")
    MANIFEST_CAPS=$(python3 -c "
import yaml
m = yaml.safe_load(open('${MANIFEST_FILE}'))
for c in m.get('capabilities', []):
    print(f\"  - {c['id']} (source: {c.get('source','?')}, reason: {c.get('reason','?')})\")
")
    echo "Context: ${MANIFEST_CONTEXT}"
    echo "Stack:   ${MANIFEST_STACK}"
    echo "Seed capabilities:"
    echo "${MANIFEST_CAPS}"
else
    DISCOVERY_MODE="A"
    echo "Mode:    A (Standalone — no manifest.yaml)"
fi
...
**Mode A (Standalone):** No manifest provided. Discover all capabilities from prompt only.
**Mode B (DESIGN-seeded):** A manifest.yaml is provided with pre-resolved capabilities from the DESIGN pipeline. You MUST:
1. Load ALL seed capabilities from the manifest — these are pre-validated and have precedence
2. Run normal discovery (R1-R9) on the prompt for ADDITIONAL capabilities not in the manifest
3. Merge: seed + prompt-discovered, no duplicates (if both resolve the same capability_id, keep the seed version)
4. Mark seed capabilities with detection_reason: "design-seeded"
5. Mark prompt-discovered capabilities with their normal detection_reason
6. Use service_name from manifest.context_id (not from prompt)
7. Apply manifest.tech_defaults as variant_selections (e.g., http_client: feign)
...
5. **In Mode B, NEVER remove seed capabilities** - You can only ADD to them
...

{
  ...
  "service_name": "{{extracted from prompt (Mode A) or manifest.context_id (Mode B)}}",
  ...
  "detected_capabilities": [
    {
      ...
      "phase": {{1 or 2 or 3}},
      "detection_reason": "{{one of: foundational, keyword, dependency, implication, design-seeded}}"
    }
  ],
  "phases": {
...
# Add mode indicator
echo "" >> "${TEMP_PROMPT}"
echo "<discovery_mode>" >> "${TEMP_PROMPT}"
echo "${DISCOVERY_MODE}" >> "${TEMP_PROMPT}"
echo "</discovery_mode>" >> "${TEMP_PROMPT}"

# Add manifest if Mode B
echo "" >> "${TEMP_PROMPT}"
echo "<design_manifest>" >> "${TEMP_PROMPT}"
if [ "${DISCOVERY_MODE}" = "B" ]; then
    cat "${MANIFEST_FILE}" >> "${TEMP_PROMPT}"
else
    echo "(not provided — Mode A)" >> "${TEMP_PROMPT}"
fi
echo "" >> "${TEMP_PROMPT}"
echo "</design_manifest>" >> "${TEMP_PROMPT}"
...
with open('${OUTPUT_FILE}') as f:
    d = json.load(f)
    mode = d.get('discovery_mode', 'A')
    print(f"Mode:         {'B (DESIGN-seeded)' if mode == 'B' else 'A (Standalone)'}")
    print(f"Service:      {d.get('service_name', 'N/A')}")
    caps = d.get('detected_capabilities', [])
    print(f"Capabilities: {len(caps)}")
    seeded = sum(1 for c in caps if c.get('detection_reason') == 'design-seeded')
    discovered = len(caps) - seeded
    if mode == 'B':
