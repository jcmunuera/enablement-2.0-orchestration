...
#
# Supports two modes:
#   Mode A (Standalone): Discovery from prompt only
#   Mode B (DESIGN-seeded): manifest.yaml present → seed capabilities from DESIGN
#
# Usage:
#   ./orchestrate.sh <inputs_dir> [base_output_dir]
#
# Where inputs_dir contains:
#   - prompt.md               (user prompt/requirements, required)
#   - domain-api-spec.yaml    (OpenAPI spec, optional)
#   - system-api-*.yaml       (System API specs, optional)
#   - mapping.json            (field mappings, optional)
#   - manifest.yaml           (DESIGN capabilities, optional — triggers Mode B)
#   - aggregate-definitions.yaml (DDD model, optional — Mode B)
#   - *.feature               (BDD scenarios, optional — Mode B)
#
...
# Extract service name from manifest (Mode B), spec, or prompt (Mode A)
...
# Mode B: Try manifest.yaml first (DESIGN-seeded)
manifest_path = os.path.join(inputs_dir, 'manifest.yaml')
...
# Mode A: Try to get from OpenAPI spec
